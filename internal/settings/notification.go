...
type NotificationSettings struct {
	...
	TwilioToken        string `db:"twilio_token"`
	TwilioSenderNumber string `db:"twilio_sender_number"`
}
...
func (notificationSettings *NotificationSettings) Save() error {
	updateNotificationSettingsQuery := db.Instance.From("open_board_notification_settings").Prepared(true).
		...
		Set(goqu.Record{
			...
			"twilio_sid":           notificationSettings.TwilioSID,
			"twilio_token":         notificationSettings.TwilioToken,
			"twilio_sender_number": notificationSettings.TwilioSenderNumber,
		}).
		Executor()
